...
`, lang, orderQuery, limitQuery, offsetQuery)
	...
	if err != nil {
		return nil, 0, web.NewRequestError(errors.Wrap(err, "selecting position"), http.StatusBadRequest)
	}
	...
	for rows.Next() {
		...
		if err = rows.Scan(&detail.ID, &detail.Name); err != nil {
			return nil, 0, web.NewRequestError(errors.Wrap(err, "scanning position"), http.StatusBadRequest)
		}
	...
	for countRows.Next() {
		if err = countRows.Scan(&count); err != nil {
			return nil, 0, web.NewRequestError(errors.Wrap(err, "scanning position count"), http.StatusBadRequest)
		}
	}
...
func (r Repository) GetDetailById(ctx context.Context, id int) (GetDetailByIdResponse, error) {
	...
	if nameByte != nil {
		if err = json.Unmarshal(nameByte, &detail.Name); err != nil {
			return GetDetailByIdResponse{}, web.NewRequestError(errors.Wrap(err, "position name unmarshall"), http.StatusBadRequest)
		}
	}
	if err != nil {
