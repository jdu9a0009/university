...
func (r Repository) GetRegionByRepublicIDList(ctx context.Context, republicID int, filter Filter) ([]GetListByRepublicIDResponse, int, error) {
	_, err := r.CheckClaims(ctx)
	if err != nil {
		return nil, 0, err
	}
	...
	var list []GetListByRepublicIDResponse

	for rows.Next() {
		var detail GetListByRepublicIDResponse
